...
export function SchedulePanel({
  ...
  const canSchedule = scheduleDate && selectedChannelCount > 0 && !isSaving;
      ...
      {selectedChannelCount > 0 ? (
        ...
        <div className="flex flex-col items-center gap-2 py-2">
          <div className="relative">
            ...
            {/* Pulsing dots around the calendar */}
            <span className="absolute -top-0.5 -right-0.5 h-1.5 w-1.5 rounded-full bg-accent/40 animate-ping" />
            <span className="absolute -bottom-0.5 -left-0.5 h-1.5 w-1.5 rounded-full bg-accent/30 animate-ping" style={{ animationDelay: '0.5s' }} />
            <span className="absolute top-1/2 -right-1.5 h-1 w-1 rounded-full bg-accent/25 animate-ping" style={{ animationDelay: '1s' }} />
          </div>
          <p className="text-xs text-text-tertiary">No platforms selected</p>
        </div>
      )}

      {/* Date/time picker — enhanced */}
